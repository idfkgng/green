...
if not os.path.exists('config.ini'):
    raise SystemExit("config.ini not found! Make sure it's in the same folder as the bot.")
config.read('config.ini')

DISCORD_BOT_TOKEN           = config.get('bot', 'token')
BOT_NAME                    = config.get('bot', 'name', fallback='Vortex Nodes')
BOT_VERSION                 = config.get('bot', 'version', fallback='v0.0.2')
BOT_PREFIX                  = config.get('bot', 'prefix', fallback='.')

MAIN_ADMIN_ID               = config.getint('discord', 'main_admin_id')
VPS_USER_ROLE_ID            = config.getint('discord', 'vps_user_role_id')

THUMBNAIL_IMAGE_URL         = config.get('images', 'thumbnail_url')
FOOTER_ICON_URL             = config.get('images', 'footer_icon_url')
MESSAGES_GIF_URL            = config.get('images', 'messages_gif_url')

DEFAULT_STORAGE_POOL        = config.get('lxc', 'default_storage_pool', fallback='default')

PAYMENT_METHODS = {
    "Bitcoin":  config.get('payment', 'bitcoin',  fallback='SOON'),
    "Ethereum": config.get('payment', 'ethereum', fallback='SOON'),
    "PayPal":   config.get('payment', 'paypal',   fallback='MAKE TICKET'),
    "CashApp":  config.get('payment', 'cashapp',  fallback='SOON'),
}

TRIAL_VPS_DURATION_DAYS     = config.getint('trial',   'duration_days',              fallback=3)
MESSAGE_REWARD              = config.getint('rewards', 'message_reward',             fallback=15)
MESSAGE_THRESHOLD           = config.getint('rewards', 'message_threshold',          fallback=50)
TIC_TAC_TOE_REWARD          = config.getint('rewards', 'tic_tac_toe_reward',         fallback=50)
ROCK_PAPER_SCISSORS_REWARD  = config.getint('rewards', 'rock_paper_scissors_reward', fallback=30)
NUMBER_GUESS_REWARD         = config.getint('rewards', 'number_guess_reward',        fallback=40)
TRIVIA_REWARD               = config.getint('rewards', 'trivia_reward',              fallback=35)
CPU_THRESHOLD               = config.getint('monitor', 'cpu_threshold',             fallback=90)
...
if not DISCORD_BOT_TOKEN or DISCORD_BOT_TOKEN == 'your_bot_token_here':
    raise SystemExit("No bot token set! Edit the [bot] token = in config.ini")
# ============= END CONFIG =============
...
# Core create function — returns dict with ip and root_password
async def core_create_container(container_name: str, ram_gb: int, cpu: int, storage_gb: int, storage_pool: str = None):
    ...
    try:
        ...
        # Create Ubuntu 22.04 container with retry logic
        max_retries = 3
        for attempt in range(max_retries):
            try:
                await execute_lxc(
                    f"lxc launch ubuntu:22.04 {container_name} "
                    f"--config limits.memory={ram_mb}MB "
                    f"--config limits.cpu={cpu} "
                    f"-s {storage_pool}",
                    timeout=600
                )
                break
        ...
        # Wait for network to come up (up to 60 seconds)
        logger.info(f"Waiting for network on {container_name}...")
        network_ready = False
        for _ in range(12):
            await asyncio.sleep(5)
            try:
                await execute_lxc(
                    f"lxc exec {container_name} -- ping -c 1 -W 3 8.8.8.8",
                    timeout=10
                )
                network_ready = True
                break
        ...
        # Fix DNS if network check failed
        if not network_ready:
            logger.warning(f"Network not ready on {container_name}, forcing DNS fix...")
            try:
                await execute_lxc(
                    f"lxc exec {container_name} -- bash -c "
                    f"\"echo 'nameserver 8.8.8.8' > /etc/resolv.conf && "
                    f"echo 'nameserver 1.1.1.1' >> /etc/resolv.conf\"",
                    timeout=15
                )
                await asyncio.sleep(3)
                await execute_lxc(f"lxc exec {container_name} -- ping -c 1 -W 5 8.8.8.8", timeout=15)
                network_ready = True
            except Exception:
                logger.warning(f"Container {container_name} may not have internet — check LXD bridge config on host")

        # Generate a secure root password
        root_password = ''.join(random.choices(string.ascii_letters + string.digits, k=16))

        # Install & configure OpenSSH with password auth for root
        ssh_setup = (
            ...
            "systemctl enable ssh && systemctl restart ssh || service ssh restart || true"
        )
        try:
            await execute_lxc(
                f"lxc exec {container_name} -- bash -c \"{ssh_setup}\"",
                timeout=300
            )
            logger.info(f"SSH configured on {container_name}")
        except Exception as e:
            logger.warning(f"SSH setup issue on {container_name}: {e}")

        # Get the container's IP address
        container_ip = "Pending"
        try:
            ip_info = await execute_lxc(f"lxc list {container_name} --format json", timeout=15)
            ip_data = json.loads(ip_info)
            if ip_data:
                network = ip_data[0].get("state", {}).get("network", {})
                for iface, net_info in network.items():
                    if iface == "lo":
                        continue
                    for addr in net_info.get("addresses", []):
                        if addr.get("family") == "inet":
                            ...
                            break
                    if container_ip != "Pending":
                        break
        except Exception as e:
            logger.warning(f"Could not fetch IP for {container_name}: {e}")

        return {"ip": container_ip, "root_password": root_password}
    ...
    class TrialConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            try:
                ...
                vps_creds = await core_create_container(
                ...
                vps_info = {
                    ...
                    'storage': 15,
                    'status': 'running',
                    'ip': vps_creds.get('ip', 'Pending'),
                    'root_password': vps_creds.get('root_password', ''),
                    'created_at': datetime.now().isoformat(),
                    'expires_at': expiry_date.isoformat(),
                    'is_trial': True
                }
                ...
                success_embed.add_field(name="🔐 SSH Access",
                    value=f"**IP:** `{vps_creds.get('ip', 'Pending')}`\n"
                          f"**User:** `root`\n"
                          f"**Password:** `{vps_creds.get('root_password', 'N/A')}`\n"
                          f"**Command:** `ssh root@{vps_creds.get('ip', 'your-ip')}`",
                    inline=False)
                success_embed.add_field(name="Management",
                    value="Use `.manage` to manage your VPS!", inline=False)
        ...
        try:
            ...
            user_embed.add_field(name="Specifications", 
                value=f"**RAM:** {ram}GB\n"
                      f"**CPU:** {cpu} cores\n"
                      f"**Storage:** {storage}GB", inline=False)
            user_embed.add_field(name="🔐 SSH Access",
                value=f"**IP:** `{vps_creds.get('ip', 'Pending')}`\n"
                      f"**User:** `root`\n"
                      f"**Password:** `{vps_creds.get('root_password', 'N/A')}`\n"
                      f"**Command:** `ssh root@{vps_creds.get('ip', 'your-ip')}`",
...
async def manage_vps(ctx):
    ...
    embed.add_field(
        ...
        value="Reply with:\n"
              ...
              "**ssh [number]** - Get SSH credentials\n"
    ...
    try:
        ...
        if action == 'start':
            ...
            try:
                ...
                # Refresh IP if it shows as Pending
                container_ip = vps.get('ip', 'Pending')
                if container_ip in ('Pending', '', None):
                    try:
                        ip_info = await execute_lxc(f"lxc list {container_name} --format json", timeout=15)
                        ip_data = json.loads(ip_info)
                        if ip_data:
                            ...
                            for iface, net_info in network.items():
                                ...
                                for addr in net_info.get("addresses", []):
                                    if addr.get("family") == "inet":
                                        ...
                                        save_data()
                                        break
                                if container_ip not in ('Pending', '', None):
                                    break
                    except Exception:
                        pass
                ...
                # If no password stored, reset it now
                if not root_password:
                    root_password = ''.join(random.choices(string.ascii_letters + string.digits, k=16))
                    try:
                        await execute_lxc(
                            f"lxc exec {container_name} -- bash -c \"echo 'root:{root_password}' | chpasswd\"",
                            timeout=15
                        )
                        vps['root_password'] = root_password
                        save_data()
                    except Exception as pw_err:
                        logger.warning(f"Could not reset password: {pw_err}")

                success_embed = create_success_embed(
                    "🔐 SSH Access Info",
                    f"Connect to your VPS `{container_name}` using the credentials below."
                )
                success_embed.add_field(name="🌐 IP Address", value=f"`{container_ip}`", inline=True)
                success_embed.add_field(name="👤 Username", value="`root`", inline=True)
                success_embed.add_field(name="🔑 Password", value=f"`{root_password}`", inline=True)
                ...
                success_embed.add_field(
                    name="📝 Notes",
                    value="• Make sure your SSH client can reach the VPS IP\n"
                          "• If the IP shows `Pending`, wait 30s and try again\n"
                          "• Use `.manage` → `info` to see the latest IP\n"
                ...
                try:
                    ...
                    await ctx.send(embed=create_success_embed(
                        "✅ SSH Info Sent!",
                        f"Check your DMs for SSH credentials for `{container_name}`!"
                    ))
                except discord.Forbidden:
                    await ctx.send(embed=success_embed)

            except Exception as e:
                logger.exception(f"SSH info fetch failed: {e}")
                await ctx.send(embed=create_error_embed("SSH Info Failed", str(e)[:150]))
            ...
            await ctx.send(embed=create_error_embed("Invalid Action", 
                "Available actions: start, stop, restart, info, ssh, delete"))
...
async def start_all(ctx):
    ...
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
            if vps.get('status') == 'running':
                continue  # Skip already running containers
            container_name = vps['container_name']
            try:
                await execute_lxc(f"lxc start {container_name}")
                vps['status'] = 'running'
            ...
            for user_id, vps_list in vps_data.items():
                for vps in vps_list:
                    ...
                    if vps.get('status') == 'suspended':
                        try:
                            await execute_lxc(f"lxc resume {container_name}")
                            vps['status'] = 'running'
                            unsuspended_count += 1
                        except Exception as e:
                            failed.append(f"{container_name}: {str(e)[:50]}")
...
async def admin_create(ctx, member: discord.Member = None, plan_name: str = "Starter"):
    ...
    try:
        ...
        try:
            user_embed = create_success_embed("✅ VPS Ready!", f"An admin created a {plan_name} VPS for you!")
            user_embed.add_field(name="🔐 SSH Access",
                value=f"**IP:** `{vps_creds.get('ip', 'Pending')}`\n"
                      f"**User:** `root`\n"
                      f"**Password:** `{vps_creds.get('root_password', 'N/A')}`\n"
...
async def show_help(ctx):
    ...
    user_cmds = (
        ...
        "**.trial** - Get FREE 3-day trial VPS (3GB RAM, 2 CPU, 15GB)\n"
